...
class DataCRUDModule(tk.Frame):
    ...
    def receive_processed_data(self, data):
        self.controller.uploaded_data = data
    ...
    def update_data_table(self):
        ...
        if self.controller.processed_data is not None:
            X, _ = self.controller.processed_data
            features = ['年纪', '性别', '购买金额', '历史购买次数', '购买频率']
            if all(col in X.columns for col in features):
    ...
    def perform_analysis(self):
        ...
        X, _ = self.controller.processed_data
        features = ['年纪', '性别', '购买金额', '历史购买次数', '购买频率']
        if not all(col in X.columns for col in features):
        ...
        male_count = gender_counts.get(1, 0)  # 注意这里是编码后的值
