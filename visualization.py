...
class VisualizationModule(tk.Frame):
    ...
    def plot_analysis_charts(self):
        self.data = self.controller.uploaded_data  # 假设数据已在DataCRUDModule处理后存储
        if self.data is None:
            messagebox.showerror("错误", "未获取到数据，请先进行数据分析")
            return
        ...
        self.plot_pie_chart(ax1, self.data["性别"], "男女比例", ["男性", "女性"])

        ax2 = self.figure.add_subplot(2, 2, 2)
        self.plot_pie_chart(ax2, self.data["年纪区间"], "年纪分布")

        ax3 = self.figure.add_subplot(2, 2, 3)
        self.plot_pie_chart(ax3, self.data["金额区间"], "购买金额区间分布")

        ax4 = self.figure.add_subplot(2, 2, 4)
        self.plot_pie_chart(ax4, self.data["RepeatPurchase"], "复购占比", ["不复购", "复购"])

        # 补充图表：历史购买次数区间柱状图（子图5）
        ax5 = self.figure.add_subplot(3, 2, 5)
        purchase_counts = self.data["购买次数区间"].value_counts().sort_index()
        ...
        frequency_counts = self.data["购买频率"].value_counts().sort_index()
